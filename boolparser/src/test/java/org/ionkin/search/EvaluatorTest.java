...
public class EvaluatorTest {
    ...
    public void wordtest() {
        int[] ids = evaluator.evaluate("полку Игореве", 50);
        int[] polk = evaluator.evaluate("«полку Игореве»", 50);
        evaluator.evaluate("«Слово о полку Игореве»", 50);
        evaluator.evaluate("«Слово && о полку && Игореве» / 4", 50);
    }
    ...
    public void evaluateNotAndOrBrace() {
        ...
        // ans == 7, 15, 16, 20, 28
        int[] indices = evaluator.evaluate(query, 5);
        assertTrue(Arrays.equals(indices, new int[]{7, 15, 16, 20, 28}));
    }

    @Test
    public void evaluateQuotes() {
        int[] indices1 = evaluator.evaluate("«полку Игореве»", 5);
        int[] indices2 = evaluator.evaluate("полку Игореве", 5);
